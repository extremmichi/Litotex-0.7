...
$game_pack_filename="game.zip";   
$setup_tmp_folder="setup_tmp";  
$lito_version ="0.7.0";
$max_step =7;
$filecounter=0;
$root_path=getcwd();
...
define("LITO_ROOT_PATH", $root_path);


if($step==1) { 

if ($_SESSION['no_unpack'] == 0){
	if ( !is_file($game_pack_filename)){
...
if (!is_dir($setup_tmp_folder)){

	@mkdir($setup_tmp_folder);
	@chmod($setup_tmp_folder, 0777);
if (!is_dir($setup_tmp_folder)){
	echo("Bitte lege einen Ordner '".$setup_tmp_folder."' an, welcher für die Installation Schreibrechte hat (777)." );
	exit();
	}
}

if (!is_writable(LITO_ROOT_PATH.$setup_tmp_folder)){
	echo("Der Ordner '".$setup_tmp_folder."' hat keine Schreibrechte.<br>Bitte setze die Rechte des Ordners mittels FTP auf 777" );
	exit();
}
...
if ($_SESSION['no_unpack'] == 0){
	...
	$ret=deltree($setup_tmp_folder);

$archive = new PclZip($game_pack_filename);
 if ($archive->extract(PCLZIP_OPT_PATH, $setup_tmp_folder) == 0) {
    die("Error : ".$archive->errorInfo(true));
    exit();
  }
}

if (!is_file("./".$setup_tmp_folder."/setup/class_template.php")){
	echo("Setup konnte nicht geladen werden. Error Code 0x6727<br>");
	exit();
	}

  require("./".$setup_tmp_folder."/setup/class_template.php");  
 $tpl=new tpl(1,1);   
	
  
  
  $over ="Litotex Setup -= Version:".$lito_version. " =-";
  $content="<span class=\"normalfont\">Dieses Setup installiert die <br>Litotex Browsergameengine auf ihrem Rechner.<br><br>Bitte wenden Sie sich im Falle von Fragen und Anregungen an unser <a href=\"http://www.freebg.de\" target=\"_blank\">Litotex Forum</a></span> ";
  $content.="<br><br>";
  ...
  $content.="*)den Urheberrechtshinweis in allen Templates, in der Form der von Litotex ausgelieferten Layoutstrukturierung anzuzeigen.<br></span> ";
 	$content.="<br><br><span class=\"normalfont_o\">Für Fragen stehen wir im Forum oder unter info@freebg.de jederzeit zur Verfügung.</span> ";
  ...
  $button="<input type=\"submit\" class=\"buttons\" name=\"submit\" value=\"weiter\">";
	...
	session_unregister('ftp_server');
	session_unregister('ftp_user');
	session_unregister('ftp_kennwo');
	session_unregister('ftp_port');
	session_unregister('ftp_ordner');
...
if($step==2) { 
	...
	require("./".$setup_tmp_folder."/setup/class_template.php"); 
	 $tpl=new tpl(1,1);   
		...
		$ftp_kennwo="kennwort";
		$ftp_port="21";
		$ftp_ordner=LITO_ROOT_PATH; ;
	
	
	$_SESSION['ftp_server']=$ftp_server;
	$_SESSION['ftp_user']=$ftp_user;
	$_SESSION['ftp_kennwo']=$ftp_kennwo;
	$_SESSION['ftp_port']=$ftp_port;
	$_SESSION['ftp_ordner']=$ftp_ordner;
	
	
	require("./".$setup_tmp_folder."/setup/ftp_class.php"); 
	...
	if (!$ftp->connected) {
		...
		$content.="Installationsordner: $ftp_ordner<br>";
		...
		$files = preg_find('/./', './setup_tmp', PREG_FIND_DIRONLY|PREG_FIND_RECURSIVE);
		$fp = fopen('./setup_tmp/dirlist.txt', 'w'); 
			foreach($files as $file){
				$file=str_replace("./".$setup_tmp_folder."/",'',$file);
				fputs($fp, $file."\n"); 

			} 
		fclose($fp);  

		$files = preg_find('/./', './setup_tmp', PREG_FIND_RECURSIVE);
		$anz=sizeof($files );
	 $filecounter=$anz;
		$fp = fopen('./setup_tmp/filelist.txt', 'w'); 
			foreach($files as $file){
				$file=str_replace("./".$setup_tmp_folder."/",'',$file);
				fputs($fp, $file."\n"); 
			} 
...
if($step==4) {	
	...
	$tpl=new tpl(1,1);   
		...
		$content="";	
/*
	@mysql_connect($sql_server, $sql_user, $sql_kennwo);
		...
		*/
	$link = @mysql_connect($sql_server, $sql_user, $sql_kennwo);
		if (mysql_error()){
			$_SESSION['error_msg']="Es konnte keine Verbindung zum SQL Server hergestellt werden.<br>Error:".mysql_error();
			header("LOCATION: setup.php?step=4");
			exit();
		}
	$db_selected = @mysql_select_db($sql_db, $link);
	if (!$db_selected) {
			$_SESSION['error_msg']="Die eingetragene Datenbank '$sql_db' konnte nicht gefunden werden.<br>Error:".mysql_error();
			header("LOCATION: setup.php?step=4");
			exit();
	}
	
				$count=0;
				$sqlfile = fopen("./".$setup_tmp_folder."/setup/db_clean.sql", 'r');
				$toexec = '';
	...
	if (mysql_error()){
					$content.="Beim importieren ist ein Fehler aufgetreten<br>". mysql_error() ;
					$_SESSION['error_msg']=mysql_error();
	}else{
	$_SESSION['error_msg']="";
	}
					fclose($sqlfile);
	...
	$content.="<span class=\"normalfont_o\">Es wurden ". $count ." Einträge in der Datenbank vorgenommen.<br><br></span>";
...
if($step==5) {	
	...
	if (isset ($_SESSION['error_msg'])) {
		...
		$ftp = new ftp($ftp_server, $ftp_user, $ftp_kennwo, $ftp_ordner, $ftp_port);
		/*
		$ftp->chown_perm(0777, $ftp_ordner."/alli_flag");
		$ftp->chown_perm(0777, $ftp_ordner."/backup");
		$ftp->chown_perm(0777, $ftp_ordner."/battle_kr");
		$ftp->chown_perm(0777, $ftp_ordner."/cache");
		$ftp->chown_perm(0777, $ftp_ordner."/image_user");
		$ftp->chown_perm(0777, $ftp_ordner."/images_sig");
		$ftp->chown_perm(0777, $ftp_ordner."/images_tmp");
		$ftp->chown_perm(0777, $ftp_ordner."/templates_c");
		$ftp->chown_perm(0777, $ftp_ordner."/templates_c/standard");
		$ftp->chown_perm(0777, $ftp_ordner."/acp/cache");
		$ftp->chown_perm(0777, $ftp_ordner."/acp/templates_c");
		$ftp->chown_perm(0777, $ftp_ordner."/acp/templates_c/standard");
		$ftp->chown_perm(0777, $ftp_ordner."/acp/tmp");
		$ftp->chown_perm(0777, $ftp_ordner."/options/options.php");
		$ftp->rm_dir($ftp_ordner."/setup");
		deltree("..".$ftp_ordner."/setup");
*/
		$ftp->chown_perm(0777, "alli_flag");
		$ftp->chown_perm(0777, "backup");
		...
		$ftp->chown_perm(0777, "templates_c");
		$ftp->chown_perm(0777, "templates_c/standard");
		$ftp->chown_perm(0777, "acp/cache");
		$ftp->chown_perm(0777, "acp/templates_c");
		$ftp->chown_perm(0777, "acp/templates_c/standard");
		$ftp->chown_perm(0777, "acp/tmp");
		$ftp->chown_perm(0777, "options/options.php");
	
		deltree("setup");
	$ftp->rm_dir("setup");

$game_path=getcwd()		;
$ftp_ordner=dirname($_SERVER['PHP_SELF']); 
$current_url="http://www.". $_SERVER['SERVER_NAME'].$ftp_ordner."/";
...
if($step==6) {	
	...
	$tpl=new tpl(1,1);   
		...
		if ($game_url==""){
				...
				exit();
					...
					$option_f_name="./includes/config.php";
			    ...
			    \$ftphost = \"".$_SESSION['ftp_server']."\";
			    \$ftpuser = \"".$_SESSION['ftp_user']."\";
			    \$ftppassword = \"".$_SESSION['ftp_kennwo']."\";
			    \$ftproot = \"".$_SESSION['ftp_ordner']."\";
			    \$ftpport = \"".$_SESSION['ftp_port']."\";
	...
	if (!$db_selected) {
		...
		$n=1;
		$result=mysql_query("SELECT * FROM cc".$n."_crand ORDER BY rand()");
   	$land=mysql_fetch_array($result);
		$md5_pw = md5($admin_kwort);
		mysql_query("INSERT INTO cc".$n."_users (username,email,password, serveradmin,register_date) VALUES ('" . $admin_name. "','','$md5_pw',  1,'".time()."')");
   	$userid_r=mysql_insert_id();
   	mysql_query("INSERT INTO cc".$n."_countries (res1,res2,res3,res4,userid,lastressources,picid,x,y,size) VALUES ('4000','4000','4000','4000','$userid_r','".time()."','1','$land[x]','$land[y]','500')");
   	$islandid_r=mysql_insert_id();

   	mysql_query("UPDATE cc".$n."_crand SET used='1' WHERE x='".$land['x']."' AND y='".$land['y']."'");

   	mysql_query("UPDATE cc".$n."_users SET activeid='$islandid_r' WHERE userid='$userid_r'");				
  	...
  	$content.="Zum Login bitte <a href=\"index.php\" target=\"_blank\">HIER</a> klicken .<br>";
		...
		$content.=".<br>";
		$content.="Hilfe und Erweiterungen gibt es in unserem <a href=\"http://www.freebg.de\" target=\"_blank\">FORUM</a>.</span> ";
  	$content.="<br><br>";
...
if($step==7) {	
deltree ("./setup_tmp");
if ( is_file("./".$game_pack_filename))unlink ("./".$game_pack_filename);
